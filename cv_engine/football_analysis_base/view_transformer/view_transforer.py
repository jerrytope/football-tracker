...
class ViewTransformer:
    def __init__(self):
        ...
        self.pixel_vertices = np.array(
            [[110, 1035], [265, 275], [910, 260], [1614, 950]]
        )

        self.taget_vertices = np.array(
            [[0, court_width], [0, 0], [court_length, 0], [court_length, court_width]]
        )

        self.pixel_vertices = self.pixel_vertices.astype(np.float32)
        self.taget_vertices = self.taget_vertices.astype(np.float32)

        self.perspective_transformer = cv2.getPerspectiveTransform(
            self.pixel_vertices, self.taget_vertices
        )
